...
from trainer import BaseTrainer, KDTrainer, MultiTrainer, TripletTrainer,IKDTrainer
...
def parse_arguments():
    ...
    parser.add_argument("--augtype", default="None", type=str,
                        help="data augmentation type. canbe None,mixup or ")
...
def setup_teacher(t_name, params):
    ...
    if params["t_checkpoint"]:
        ...
        teacher_trainer = BaseTrainer(t_net,None, config=teacher_config)
    ...
    print("Teacher Accuracy:",best_t_acc)
...
def test_nokd(s_net, t_net, params):
    ...
    nokd_trainer = BaseTrainer(s_net,t_net=t_net, config=nokd_config)
    best_acc = nokd_trainer.train()
    return best_acc



def test_ikd(s_net, t_net, params):
    print("---------- Training input IKD -------")
    ikd_config = params.copy()
    ikd_trainer = IKDTrainer(s_net,t_net=t_net, config=ikd_config)
    best_acc = ikd_trainer.train()
    return best_acc
...
def run_benchmarks(modes, params, s_name, t_name):
    ...
    for mode in modes:
        ...
        # start the test
        print('sdada',globals()[f"test_{mode}"])
        try:
